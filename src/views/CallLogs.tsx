...
const initialCalls: CallRecord[] = [
  {
    ...
    dateTime: 'May 21, 2024 10:30 AM',
    recordingLength: '06:32',
    source: 'Website',
    aiSummary: 'Rahul called to discuss the pricing for the Enterprise plan. He is interested in a custom API integration and requested a demo for his development team. Follow-up scheduled.',
    keyPoints: [
      'Interested in Enterprise plan custom quote.',
      'Requested technical API integration walkthrough.',
      'Needs live demo for a team of 8-10 members.',
      'Budget negotiations deferred to next follow-up.'
    ],
  ...
  },
  {
    ...
    dateTime: 'May 21, 2024 09:15 AM',
    recordingLength: '04:18',
    source: 'Lead Form',
    aiSummary: 'Outbound check-in with Amit Verma regarding trial feedback. He is satisfied with the AI Leads module but wants to verify WhatsApp pricing tiers before committing.',
    keyPoints: [
      'Positive trial feedback on Lead scoring.',
      'Inquired about WhatsApp broadcast limits.',
      'Requested standard contracts and SLAs.'
    ],
    nextAction: 'Email pricing sheet'
  },
  {
    id: '3',
    ...
    dateTime: 'May 21, 2024 08:45 AM',
    recordingLength: '—',
    source: 'Google Ads',
    aiSummary: 'Missed inbound call. Customer had signed up for trial 20 minutes prior. Likely requires onboarding support.',
    keyPoints: [
      'Inbound call abandoned after 15 seconds.',
      'Contact is a recently signed-up free trial user.'
    ],
    nextAction: 'Return callback immediately'
  },
  {
    id: '4',
    ...
    status: 'Answered',
    duration: '07:48',
    agentName: 'Rahul Desai',
    agentAvatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=150',
    dateTime: 'May 20, 2024 07:30 PM',
    recordingLength: '07:48',
    source: 'Cold Outreach',
    aiSummary: 'Follow-up regarding proposal sent last week. Dinesh is reviewing contract clauses with legal and needs a slight adjustment to payment terms.',
    keyPoints: [
      'Proposal under review by Globex legal team.',
      'Requested net-45 billing terms instead of net-30.',
      'Ready to sign contract once payment terms are revised.'
    ],
    nextAction: 'Revise proposal terms'
  },
  {
    id: '5',
    ...
    agentName: 'Priya Mehta',
    agentAvatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=150',
    dateTime: 'May 20, 2024 06:20 PM',
    recordingLength: '03:11',
    source: 'Referral',
    aiSummary: 'Neha wanted to schedule an upgrade for their current standard tier. Priya verified billing details and guided her through the automated checkout portal.',
    keyPoints: [
      'Account expansion: upgrading from Standard to Premium.',
      'Add-on: requested 2 additional admin seats.',
      'Billing profile updated successfully.'
    ],
    nextAction: 'Confirm dashboard upgrade'
  }
];

export default function CallLogs() {
  const [calls] = useState<CallRecord[]>(initialCalls);
  const [selectedCall, setSelectedCall] = useState<CallRecord>(initialCalls[0]);
        ...
        <div className="flex-1 w-full bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-wrap gap-1 bg-slate-50 p-1 rounded-xl">
              {[
                { id: 'all', label: 'All Calls', count: 1248 },
                { id: 'inbound', label: 'Inbound', count: 698 },
                { id: 'outbound', label: 'Outbound', count: 550 },
                { id: 'missed', label: 'Missed', count: 356 }
              ].map((tab) => (
                <button
                  key={tab.id}
                ...
                {filteredCalls.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3">
                        <span className="text-slate-800 font-bold block">{c.contactName}</span>
                        <span className="text-[10px] text-slate-400 block">{c.company}</span>
                      </td>
                      <td className="py-3">
                      ...
                      <td className="py-3 text-slate-500 font-medium">{c.duration}</td>
                      <td className="py-3">
                        <div className="flex items-center gap-1.5">
                          <img src={c.agentAvatar} alt={c.agentName} className="w-5 h-5 rounded-full object-cover" />
                          <span className="text-slate-600">{c.agentName}</span>
                        </div>
                      </td>
                      <td className="py-3 text-slate-400">{c.dateTime}</td>
                      <td className="py-3" onClick={(e) => e.stopPropagation()}>
                        {c.recordingLength !== '—' ? (
                          <button
                            onClick={() => {
                            ...
                            className="flex items-center gap-1 text-[10px] bg-slate-50 border border-slate-200 px-2 py-1 rounded hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all font-bold"
                          >
        ...
        {selectedCall && (
          <div className="w-full lg:w-80 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden shrink-0 flex flex-col">
            ...
            <div className="p-4 flex flex-col items-center text-center border-b border-slate-50">
              ...
              <h3 className="font-bold text-slate-800 text-sm m-0">{selectedCall.contactName}</h3>
              <span className="text-[10px] text-indigo-600 font-semibold mt-0.5">{selectedCall.company}</span>
              <span className="text-[10px] text-slate-400 mt-1 block">{selectedCall.phone} • {selectedCall.location}</span>
              ...
              {sidebarTab === 'summary' && (
                <div className="space-y-4 text-xs">
                  ...
                  <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-3 rounded-xl border border-indigo-100">
                    ...
                    <p className="text-slate-600 leading-relaxed text-[11px] font-medium">{selectedCall.aiSummary}</p>
                  </div>

                  {/* Key points */}
                  <div>
                    <span className="font-bold text-slate-800 block mb-2">Key Points</span>
                    <ul className="space-y-1.5 pl-0">
                      {selectedCall.keyPoints.map((pt, i) => (
                        <li key={i} className="flex gap-1.5 items-start text-slate-600 text-[11px] leading-relaxed">
                  ...
                  <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
                    <span className="font-bold text-slate-800">Suggested Next Action</span>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex flex-col items-center">
                        ...
                        return (
                          <div
                            ...
                            className={`w-1 rounded-full transition-all duration-300 ${
                              isPlayed ? 'bg-indigo-600 h-' + h / 5 : 'bg-slate-200 h-' + h / 5
                            }`}
                    ...
                    <div className="flex items-center gap-4 w-full justify-center">
                      ...
                      <button
                        ...
                        className="w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full flex items-center justify-center shadow-md active:scale-95 transition-all"
                      >
                        {isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} className="translate-x-0.5" fill="currentColor" />}
                      </button>
                      <button className="text-slate-400 hover:text-slate-600" title="Download recording">
                        <Download size={16} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="text-[10px] text-slate-400 space-y-1">
                    <div className="flex justify-between">
                      <span>Call ID:</span>
                      <span className="font-mono text-slate-600">CL-20240521-{selectedCall.id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Connection:</span>
                      <span className="text-slate-600">Direct Route ({selectedCall.source})</span>
                    </div>
                  </div>
                </div>
              )}
